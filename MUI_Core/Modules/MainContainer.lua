-- bottom part of the UI. Includes:
-- Unit Frame background art, action bar background art
-- space for rep bar, artifact bar, datatext
...
function private.ArtifactBar_OnSetup(resourceBar, data)
    ...
    local handler = em:CreateEventHandler("ARTIFACT_XP_UPDATE", function()
        ...
        local _, currentValue, maxValue = MainMenuBar_GetNumArtifactTraitsPurchasableFromXP(pointsSpent, totalXP, tier);
        data.statusbar:SetMinMaxValues(0, maxValue);
        data.statusbar:SetValue(currentValue);
        if (data.statusbar.text) then
            local percent = (currentValue / maxValue) * 100;
...
function private.ReputationBar_OnSetup(resourceBar, data)
    data.statusbar:HookScript("OnEnter", function(self)
        local faction_name, standingID, minValue, maxValue, currentValue = GetWatchedFactionInfo();
        if (standingID == 8) then return; end
        ...
        currentValue = tk:FormatNumberString(currentValue);
        maxValue = tk:FormatNumberString(maxValue);
        local text = tk.string.format("%s: %s / %s (%d%%)", faction_name, currentValue, maxValue, percent);
    ...
    local handler = em:CreateEventHandler("UPDATE_FACTION", function()
        local faction_name, standingID, minValue, maxValue, currentValue = GetWatchedFactionInfo();
        if (not faction_name or standingID == 8) then
            bui.Resources:DisableBar(data.id);
            return;
