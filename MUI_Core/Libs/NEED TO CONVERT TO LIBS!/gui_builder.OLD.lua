...
do
    local function DynamicScrollBar(self)
        ...
        local childHeight = tk.math.floor(child:GetHeight() + 0.5);
        local scrollHeight = tk.math.floor(self:GetHeight() + 0.5);

        if (child and childHeight and childHeight > scrollHeight) then
    ...
    function gui:CreateScrollFrame(parent, global, child_right_padding)
        ...
        container.ScrollFrame.ScrollBar:ClearAllPoints();
        container.ScrollFrame.ScrollBar:SetPoint("TOPLEFT", container.ScrollFrame, "TOPRIGHT", -7, -2);
        container.ScrollFrame.ScrollBar:SetPoint("BOTTOMRIGHT", container.ScrollFrame, "BOTTOMRIGHT", -2, 2);
        tk:SetBackground(container.ScrollFrame.ScrollBar, 0, 0, 0, 0.2);
        tk:KillElement(container.ScrollFrame.ScrollBar.ScrollUpButton);
        tk:KillElement(container.ScrollFrame.ScrollBar.ScrollDownButton);

        local r, g, b = tk:GetThemeColor();
        container.ScrollFrame.ScrollBar.thumb = container.ScrollFrame.ScrollBar:GetThumbTexture();
        container.ScrollFrame.ScrollBar.thumb:SetColorTexture(r, g, b, 0.8);
        container.ScrollFrame.ScrollBar.thumb:SetSize(5, 50);

        container.ScrollFrame:SetScript("OnShow", DynamicScrollBar);
        tk.hooksecurefunc("ScrollFrame_OnScrollRangeChanged", DynamicScrollBar);
        container.ScrollFrame.ScrollBar:Hide();
...
function Panel:SetDevMode(data, devMode)
