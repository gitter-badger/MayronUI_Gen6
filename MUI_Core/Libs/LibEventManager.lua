...
local db = LibStub:NewLibrary("LibEventManager", 1.0);
...
-- Handler Prototype
------------------------
local Handler = tk:CreateClass("Handler"); -- renamed from CreateProjectedPrototype

-- runs before data is destroyed
-- remove handler from events table
function Handler:__Hook__Destroy(data)
    if (data.event) then
        table.remove(private.events[data.event], self:GetPriority());
...
function em:CreateEventHandlers(events, func, unit)
    local handlers = {};
    for id, event in tk.ipairs(events) do
